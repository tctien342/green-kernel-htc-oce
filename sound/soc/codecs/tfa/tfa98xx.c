...
static struct tfa98xx *tfa98xx_dev = NULL; 
static int previews_profile = 7; 
			       ...
			       struct snd_ctl_elem_value *ucontrol)
{
	...
	previews_profile = tfa98xx_profile; 
	...
	for (prof = 0; prof < nprof; prof++) {
		...
		if (is_profile_in_list(bprofile->basename, bprofile->len) == 0 &&
			...
			if (tfacont_get_max_vstep(0, prof)) {
				...
				tfa98xx_controls[mix_index].private_value = prof; 
				mix_index++;
			}
		}
...
static void tfa98xx_loading_work(struct work_struct *work) 
{
	const struct firmware *cont = NULL; 
	struct tfa98xx *tfa98xx = container_of(work, struct tfa98xx, loading_work.work); 
	int container_size;
	int handle;
	int ret;
...
static int tfa98xx_load_container(struct tfa98xx *tfa98xx)
{
	...
	return queue_delayed_work(tfa98xx->tfa98xx_wq, &tfa98xx->loading_work, 0); 
}
...
static void tfa98xx_dsp_init(struct tfa98xx *tfa98xx)
{
	...
	if (tfa98xx->init_count < TF98XX_MAX_DSP_START_TRY_COUNT) {
		...
		for(noclk_count = 0; noclk && (noclk_count < 20); noclk_count++) { 
		...
		if (ret == Tfa98xx_Error_Ok) {
			...
			if(tfa98xx_profile == 7) {
				...
				for(check_count = 0; (manstate != 9) && (check_count < 10); check_count++) { 
...
static int tfa98xx_probe(struct snd_soc_codec *codec)
{
	...
	INIT_DELAYED_WORK(&tfa98xx->loading_work, tfa98xx_loading_work); 
...
static int tfa98xx_remove(struct snd_soc_codec *codec)
{
	...
	cancel_delayed_work_sync(&tfa98xx->init_work);
	cancel_delayed_work_sync(&tfa98xx->tapdet_work);
	cancel_delayed_work_sync(&tfa98xx->loading_work); 
...
{
	struct tfa98xx *tfa98xx = tfa98xx_dev;
