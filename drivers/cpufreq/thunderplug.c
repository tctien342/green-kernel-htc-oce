...
static unsigned int last_load[4] = {0, 0, 0, 0}; //Msm8916 ify
...
if(shreesha == 1) {
shreesha = 0;
pr_info("%s: Return 0 in case if user enters 1 because who uses Quad core mode on a Quad core Device so force set it to default again/n", THUNDERPLUG);
}
return shreesha;
}
...
{
	unsigned int cpu;
	endurance_level = rita(endurance_level);
...
static void __cpuinit tplug_work_fn(struct work_struct *work)
{
	...
	break;
	case 2:
		core_limit = 2; //According to maths it should be one but according to me we should be good to keep 2 cpus online for msm8916 <harshitjain6751@gmailcom>
	break;
	default:
		core_limit = 4;
	break;
...
MODULE_AUTHOR("Varun Chitre <varun.chitre15@gmail.com> && Dev-harsh1998 <harshitjain6751@gmail.com>");
MODULE_DESCRIPTION("Hotplug driver for Tuned for Quad-core CPU");
